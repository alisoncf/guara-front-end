...
export async function pesquisarObjetosFisicos(obj: ObjetoFisico) {
  ...
  try {
    ...
    listaObj.value = response.data.results.bindings.map((item: any) => ({
      ...
      colecao: dividirLista(item.colecao?.value)[0] || '',
      colecaoLista: dividirLista(item.colecao?.value),
      colecaoListaAbreviada: dividirLista(item.colecao?.value).map(
        (colecao: string) => textoAposUltimoChar(colecao, '#')
      ),
